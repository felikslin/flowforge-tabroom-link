...
export function ChatTab() {
  ...
  const chips = [
    "What's my current record?",
    "Analyze my speaker points trend",
        ...
        <div ref={msgsRef} className="flex flex-col gap-3 max-h-[300px] overflow-y-auto mb-3">
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-2 items-start ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
              <div className={`w-[26px] h-[26px] rounded-full flex-shrink-0 flex items-center justify-center text-[11px] ${
                msg.role === "assistant" ? "bg-primary text-primary-foreground" : "bg-flow-surface2 border border-border"
              }`}>
                {msg.role === "assistant" ? "✦" : initial}
              </div>
              <div className={`px-3.5 py-2.5 text-xs leading-[1.7] max-w-[84%] ${
                msg.role === "user"
          ...
          {loading && (
            <div className="flex gap-2 items-start">
              <div className="w-[26px] h-[26px] rounded-full flex-shrink-0 flex items-center justify-center text-[11px] bg-primary text-primary-foreground">✦</div>
